...
if ( !class_exists('Puc_v4p5_Factory', false) ):
	...
	class Puc_v4p5_Factory {
		...
		public static function buildUpdateChecker($metadataUrl, $fullPath, $slug = '', $checkPeriod = 12, $optionName = '', $muPluginFile = '') {
			...
			$api_params = array(
				'halim_action' 		=> 'halim_check_plugin_update', 
				'secret_key' 		=> '5a940894d20363.68764531', 
				'license_key' 		=> get_option('halim_license_key'), 
				'registered_domain' => $_SERVER['HTTP_HOST']
			);

			$jsonApi = esc_url_raw(add_query_arg($api_params, 'http://api.halimthemes.com')); 
			$jsonApi = json_decode(file_get_contents($jsonApi));

			if(isset($jsonApi->result) && $jsonApi->result == 'success') 
				$metadataUrl = $jsonApi->api_url;
